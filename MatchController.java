...
import Cricket_Game.model.Team;
...
public class MatchController {
    ...
    //static int totalOversOn1stInn;
    //static int totalOversOn2ndInn;
    ...
    public int playInnings(Team bat, Team bowl) {
        ...
        while (batsman < 10) {
           ...
           // bowler=selectBowlerToBowl(bowl);
            for (int balls = 0; balls < 6; balls++) {
                int runs = Utils.doRandom(8);
               // System.out.println("Random numbers: "+runs);
                if (batsman < 10) {
                    if (runs < 7) {
            ...
            setOvers(bat,bowl,batsman,bowler);
                bat.setTotalOversTeamPlayed();
            }
        return innScore;
    }
    //overloading
    ...
    void innSummary(Team bat, int innScore) {
        Utils.printMessage("\n" + bat.getTeamName() + " Scored " + innScore + " for "+bat.getTotalOversTeamPlayed()+"\n");
    }
    ...
    void scoreCard(Team team){
        ...
        for(int player=0;player<11;player++){
            Utils.printMessage("Player: "+team.players[player].getName()+"("+team.playerInMatch[player].getPlayerRole()+")"+" Score: "+team.playerInMatch[player].getScoredOnBatting()+" Wickets: "+team.playerInMatch[player].getWicketTaken()+" Overs: "+team.playerInMatch[player].getOversBowled());
        }
    }
    void setOvers(Team bat,Team bowl,int batsman,int bowler){
        bowl.playerInMatch[bowler].setOversBowled();
        bat.playerInMatch[batsman].setOversPlayed();
    }
}
